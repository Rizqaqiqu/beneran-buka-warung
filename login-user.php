...
if(isset($_POST['submit'])){
    ...
    if(mysqli_num_rows($cek) > 0){
        $u = mysqli_fetch_object($cek);
        if(password_verify($pass, $u->user_password)){
...
<head>
    ...
    <title>Login Pembeli - WarungRizqi</title>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        html, body { height: 100%; font-family: Inter, system-ui, Arial; background: linear-gradient(135deg, #0f172a 0%, #1a1f3a 50%, #0d1b2a 100%); color: #e6eef8; overflow: hidden; position: relative; }
        ...
        .logo-icon { width: 70px; height: 70px; background: linear-gradient(135deg, var(--primary), var(--secondary)); border-radius: 15px; display: flex; align-items: center; justify-content: center; margin: 0 auto 15px; font-size: 35px; box-shadow: 0 10px 30px rgba(79,70,229,0.3); animation: rotate 3s linear infinite; }
        ...
        input[type="email"], input[type="password"] { width: 100%; padding: 14px 16px; border-radius: 12px; border: 1px solid rgba(255,255,255,0.1); background: rgba(255,255,255,0.03); color: #e6eef8; font-family: inherit; font-size: 14px; transition: all 0.3s ease; outline: none; }
        input[type="email"]:focus, input[type="password"]:focus { border-color: rgba(79,70,229,0.5); background: rgba(255,255,255,0.06); box-shadow: 0 0 20px rgba(79,70,229,0.2); }
    ...
    <div class="container">
        <div class="login-card">
            <div class="logo-header">
                ...
                <div class="logo-subtitle">Login Pembeli</div>
            </div>
            ...
            <form method="POST">
                ...
                <button type="submit" name="submit" class="btn-login">Login Sekarang</button>
            </form>
            
            <div class="footer-link">
                Belum punya akun? <a href="register-user.php">Daftar Sekarang</a>
            </div>
        </div>
    </div>
</body>
